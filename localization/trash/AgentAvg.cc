...
AgentGyro::AgentGyro(ParticleFilter *pf) : Agent(pf)
{
}
...
void AgentGyro::doAction(void)
{
	...
	for(int i=0;i<8;i++){
		...
		if(i==0 || i==2 || i==3 || i==5){
			...
			m_pf->motionUpdate(180.0,diff);
		}else if(i==1 || i==4 || i==6 || i==7){
			Actions::turn(90);
			SensorGyro::update();
			double diff = SensorGyro::getDeltaDeg();
			m_pf->motionUpdate(0.0,diff);
		}
...
double AgentGyro::thetaValue(double target_dir_deg, double t_deg)
{
	if(target_dir_deg < 0.0 || target_dir_deg > 360.0){
		cerr << "too large target_dir_deg at AgentGyro::thetaValue" << endl;
		exit(1);
	}
...
double AgentGyro::valueFunction(double x,double y,double t_deg)
{
	if(t_deg < 0.0 || t_deg > 360.0){
		cerr << "too large t_deg at AgentGyro::valueFunction" << endl;
		exit(1);
	}
...
bool AgentGyro::canGoForward(double x,double y,double t_deg)
{
	if(t_deg < 0.0 || t_deg > 360.0){
		cerr << "too large t_deg at AgentGyro::valueFunction" << endl;
		exit(1);
